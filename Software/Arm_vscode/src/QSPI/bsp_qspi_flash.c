...
static inline uint8_t * qspi_flash_ptr(uint32_t offset)
{
    return (uint8_t *) (uintptr_t) ((uintptr_t) QSPI_DEVICE_START_ADDRESS + (uintptr_t) offset);
}

static void qspi_d0_byte_write_standard(uint8_t byte);
static fsp_err_t qspi_flash_read_direct(spi_flash_ctrl_t * p_ctrl,
                                        uint8_t          * p_dest,
...
{
    R_QSPI_Write(&g_qspi0_flash_ctrl, pBuffer, qspi_flash_ptr(WriteAddr), NumByteToWrite);
    QSPI_Flash_WaitForWriteEnd();
}
...
void QSPI_Flash_BufferWrite(uint8_t *pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
    ...
    if (Addr == 0)
    {
        ...
        else /* NumByteToWrite > SPI_FLASH_PageSize */
        {
            ...
            while (NumOfPage--)
            {
                R_QSPI_Write(&g_qspi0_flash_ctrl, pBuffer, qspi_flash_ptr(WriteAddr), SPI_FLASH_PageSize);
                QSPI_Flash_WaitForWriteEnd();
            ...
            /*若有多余的不满一页的数据，把它写完*/
            R_QSPI_Write(&g_qspi0_flash_ctrl, pBuffer, qspi_flash_ptr(WriteAddr), NumOfSingle);
            QSPI_Flash_WaitForWriteEnd();
    ...
    else
    {
        ...
        if (NumOfPage == 0)
        {
            ...
            if (NumOfSingle > count)
            {
                ...
                /*先写满当前页*/
                R_QSPI_Write(&g_qspi0_flash_ctrl, pBuffer, qspi_flash_ptr(WriteAddr), count);
                QSPI_Flash_WaitForWriteEnd();
                ...
                pBuffer += count;
                /*再写剩余的数据*/
                R_QSPI_Write(&g_qspi0_flash_ctrl, pBuffer, qspi_flash_ptr(WriteAddr), temp);
                QSPI_Flash_WaitForWriteEnd();
...
{
    qspi_flash_read_direct(&g_qspi0_flash_ctrl, pBuffer, qspi_flash_ptr(ReadAddr), NumByteToRead);
}
                                        ...
                                        uint32_t           byte_count)
{
    ...
    uint32_t chip_address =
        (uint32_t) ((uintptr_t) p_device_address - (uintptr_t) QSPI_DEVICE_START_ADDRESS + (uintptr_t) R_QSPI->SFMCNT1);
    ...
    while (index < byte_count)
    {
        ...
        *(p_dest + index) = (uint8_t) R_QSPI->SFMCOM;
        index++;
    }
