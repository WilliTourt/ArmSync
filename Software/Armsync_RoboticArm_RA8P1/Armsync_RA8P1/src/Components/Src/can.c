...
{
    canfd_instance_ctrl_t * p_ctrl = (canfd_instance_ctrl_t *) g_canfd1.p_ctrl;
...
void can_Init(void)
{
    ...
    fsp_err_t err = g_canfd1.p_api->open(g_canfd1.p_ctrl, g_canfd1.p_cfg);
    g_can_last_err = err;
    if (FSP_SUCCESS != err)
    {
        return;
     ...
     */
    /* DEBUG: print actual bit timing register values */

   err = g_canfd1.p_api->modeTransition(g_canfd1.p_ctrl, CAN_OPERATION_MODE_NORMAL, CAN_TEST_MODE_DISABLED);
    g_can_last_err = err;
}
...
void can_SendCmd(__IO uint8_t * cmd, uint8_t len)
{
    ...
    while (i < j)
    {
        ...
        fsp_err_t err = g_canfd1.p_api->write(g_canfd1.p_ctrl, 0U, (can_frame_t *) &can.CAN_TxMsg);
        if (FSP_SUCCESS != err)
        {
            g_can_tx_done  = true;
            g_can_last_err = err;
