...
#include "hal_data.h"
agt_instance_ctrl_t agt_Poll_ctrl;
const agt_extended_cfg_t agt_Poll_extend = { .count_source = AGT_CLOCK_PCLKB,
...
const timer_cfg_t agt_Poll_cfg = { .mode = TIMER_MODE_PERIODIC,
/* Actual period: 0.008388608 seconds. Actual duty: 50%. */.period_counts =
		(uint32_t) 0x10000, .duty_cycle_counts = 0x8000, .source_div =
		(timer_source_div_t) 3, .channel = 1, .p_callback = POLL_Callback,
/** If NULL then do not add & */
    ...
    .p_context           = NULL,
		...
		.p_context = (void*) &NULL,
#endif
		.p_extend = &agt_Poll_extend, .cycle_end_ipl = (4),
...
const timer_instance_t agt_Poll = { .p_ctrl = &agt_Poll_ctrl, .p_cfg =
		&agt_Poll_cfg, .p_api = &g_timer_on_agt };
