...
void eskf_update_mag(ESKF_State* s, float mx, float my, float mz)
{
    ...
    // 4. 叉积误差
    float ex = my*pred_mz - mz*pred_my;
    float ey = mz*pred_mx - mx*pred_mz;
    float ez = mx*pred_my - my*pred_mx;
    
    // 5. 动态增益（可调）
    float mag_gain = 0.0004f;   // 小于加速度计增益（0.14），避免干扰roll/pitch
